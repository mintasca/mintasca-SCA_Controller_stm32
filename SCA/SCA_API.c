...
//V1.5.7 2022.5.24 Fix heartbeat bug for 48V SCA
...
uint8_t setActuatorID(uint8_t currentID, uint8_t newID,uint8_t isBlock)
{
	...
	/* 利用缓存位当做更新标志位 */
	pSCA->paraCache.ID = Actr_Disable;
	...
	/* 等待执行结果 */
	while((pSCA->paraCache.ID != Actr_Enable) && (waitime++ < CanOvertime));
