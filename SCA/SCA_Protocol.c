...
uint8_t SCA_Write_5(SCA_Handler_t* pSCA, uint8_t cmd, uint8_t TxData)
{
	...
	/*
		第五类写入命令数据格式：
		1字节命令+6字节地址（SCA的序列号）+1字节参数（目标数据）
	*/
	TxBuf[0] = cmd;
	...
	TxBuf[5] = pSCA->Serial_Num[4];
	TxBuf[6] = pSCA->Serial_Num[5];
	TxBuf[7] = TxData;

	return canTransmit(pSCA, TxBuf, 8);
}
...
static void R5dataProcess(SCA_Handler_t* pSCA, CanRxMsg* RxMsg)
{
	/* 装填序列号 */
	for(int i = 0; i < 6; i++)
		pSCA->Serial_Num[i] = RxMsg->Data[i + 1];
